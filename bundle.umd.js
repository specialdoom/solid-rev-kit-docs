(function(zM){typeof define=="function"&&define.amd?define(zM):zM()})(function(){"use strict";const zM=(M,N)=>M===N,jN=Symbol("solid-proxy"),tM={equals:zM};let zN=fM;const J={},Y=1,OM=2,SM={owned:null,cleanups:null,context:null,owner:null};var l=null;let X=null,O=null,_=null,x=null,p=null,oM=0;function uM(M,N){N&&(l=N);const D=O,g=l,j=M.length===0?SM:{owned:null,cleanups:null,context:null,owner:g};l=j,O=null;try{return sM(()=>M(()=>EM(j)),!0)}finally{O=D,l=g}}function b(M,N){N=N?Object.assign({},tM,N):tM;const D={value:M,observers:null,observerSlots:null,pending:J,comparator:N.equals||void 0},g=j=>(typeof j=="function"&&(j=j(D.pending!==J?D.pending:D.value)),LM(D,j));return[YM.bind(D),g]}function tN(M,N,D){const g=xM(M,N,!0,Y);H(g)}function E(M,N,D){const g=xM(M,N,!1,Y);H(g)}function m(M,N,D){D=D?Object.assign({},tM,D):tM;const g=xM(M,N,!0,0);return g.pending=J,g.observers=null,g.observerSlots=null,g.comparator=D.equals||void 0,H(g),YM.bind(g)}function uN(M){if(_)return M();let N;const D=_=[];try{N=M()}finally{_=null}return sM(()=>{for(let g=0;g<D.length;g+=1){const j=D[g];if(j.pending!==J){const z=j.pending;j.pending=J,LM(j,z)}}},!1),N}function R(M){let N,D=O;return O=null,N=M(),O=D,N}function hM(M){return l===null||(l.cleanups===null?l.cleanups=[M]:l.cleanups.push(M)),M}function TN(M){const N=Symbol("context");return{id:N,Provider:nN(N),defaultValue:M}}function iN(M){return $M(l,M.id)||M.defaultValue}function IN(M){const N=m(M);return m(()=>aM(N()))}function YM(){const M=X;if(this.sources&&(this.state||M)){const N=x;x=null,this.state===Y||M?H(this):UM(this),x=N}if(O){const N=this.observers?this.observers.length:0;O.sources?(O.sources.push(this),O.sourceSlots.push(N)):(O.sources=[this],O.sourceSlots=[N]),this.observers?(this.observers.push(O),this.observerSlots.push(O.sources.length-1)):(this.observers=[O],this.observerSlots=[O.sources.length-1])}return this.value}function LM(M,N,D){if(M.comparator&&M.comparator(M.value,N))return N;if(_)return M.pending===J&&_.push(M),M.pending=N,N;let g=!1;return M.value=N,M.observers&&M.observers.length&&sM(()=>{for(let j=0;j<M.observers.length;j+=1){const z=M.observers[j];g&&X.disposed.has(z),z.pure?x.push(z):p.push(z),z.observers&&(g&&!z.tState||!g&&!z.state)&&bM(z),g||(z.state=Y)}if(x.length>1e6)throw x=[],new Error},!1),N}function H(M){if(!M.fn)return;EM(M);const N=l,D=O,g=oM;O=l=M,AN(M,M.value,g),O=D,l=N}function AN(M,N,D){let g;try{g=M.fn(N)}catch(j){mM(j)}(!M.updatedAt||M.updatedAt<=D)&&(M.observers&&M.observers.length?LM(M,g):M.value=g,M.updatedAt=D)}function xM(M,N,D,g=Y,j){const z={fn:M,state:g,updatedAt:null,owned:null,sources:null,sourceSlots:null,cleanups:null,value:N,owner:l,context:null,pure:D};return l===null||l!==SM&&(l.owned?l.owned.push(z):l.owned=[z]),z}function pM(M){const N=X;if(M.state!==Y)return M.state=0;if(M.suspense&&R(M.suspense.inFallback))return M.suspense.effects.push(M);const D=[M];for(;(M=M.owner)&&(!M.updatedAt||M.updatedAt<oM);)(M.state||N)&&D.push(M);for(let g=D.length-1;g>=0;g--)if(M=D[g],M.state===Y||N)H(M);else if(M.state===OM||N){const j=x;x=null,UM(M,D[0]),x=j}}function sM(M,N){if(x)return M();let D=!1;N||(x=[]),p?D=!0:p=[],oM++;try{return M()}catch(g){mM(g)}finally{cN(D)}}function cN(M){x&&(fM(x),x=null),!M&&(p.length?uN(()=>{zN(p),p=null}):p=null)}function fM(M){for(let N=0;N<M.length;N++)pM(M[N])}function UM(M,N){M.state=0;const D=X;for(let g=0;g<M.sources.length;g+=1){const j=M.sources[g];j.sources&&(j.state===Y||D?j!==N&&pM(j):(j.state===OM||D)&&UM(j,N))}}function bM(M){const N=X;for(let D=0;D<M.observers.length;D+=1){const g=M.observers[D];(!g.state||N)&&(g.state=OM,g.pure?x.push(g):p.push(g),g.observers&&bM(g))}}function EM(M){let N;if(M.sources)for(;M.sources.length;){const D=M.sources.pop(),g=M.sourceSlots.pop(),j=D.observers;if(j&&j.length){const z=j.pop(),u=D.observerSlots.pop();g<j.length&&(z.sourceSlots[u]=g,j[g]=z,D.observerSlots[g]=u)}}if(M.owned){for(N=0;N<M.owned.length;N++)EM(M.owned[N]);M.owned=null}if(M.cleanups){for(N=0;N<M.cleanups.length;N++)M.cleanups[N]();M.cleanups=null}M.state=0,M.context=null}function mM(M){throw M}function $M(M,N){return M&&(M.context&&M.context[N]!==void 0?M.context[N]:M.owner&&$M(M.owner,N))}function aM(M){if(typeof M=="function"&&!M.length)return aM(M());if(Array.isArray(M)){const N=[];for(let D=0;D<M.length;D++){const g=aM(M[D]);Array.isArray(g)?N.push.apply(N,g):N.push(g)}return N}return M}function nN(M){return function(D){let g;return tN(()=>g=R(()=>(l.context={[M]:D.value},IN(()=>D.children)))),g}}const rN=Symbol("fallback");function WM(M){for(let N=0;N<M.length;N++)M[N]()}function yN(M,N,D={}){let g=[],j=[],z=[],u=0,t=N.length>1?[]:null;return hM(()=>WM(z)),()=>{let i=M()||[],I,T;return R(()=>{let r=i.length,s,a,jM,yM,lM,d,Q,h,F;if(r===0)u!==0&&(WM(z),z=[],g=[],j=[],u=0,t&&(t=[])),D.fallback&&(g=[rN],j[0]=uM(Ye=>(z[0]=Ye,D.fallback())),u=1);else if(u===0){for(j=new Array(r),T=0;T<r;T++)g[T]=i[T],j[T]=uM(c);u=r}else{for(jM=new Array(r),yM=new Array(r),t&&(lM=new Array(r)),d=0,Q=Math.min(u,r);d<Q&&g[d]===i[d];d++);for(Q=u-1,h=r-1;Q>=d&&h>=d&&g[Q]===i[h];Q--,h--)jM[h]=j[Q],yM[h]=z[Q],t&&(lM[h]=t[Q]);for(s=new Map,a=new Array(h+1),T=h;T>=d;T--)F=i[T],I=s.get(F),a[T]=I===void 0?-1:I,s.set(F,T);for(I=d;I<=Q;I++)F=g[I],T=s.get(F),T!==void 0&&T!==-1?(jM[T]=j[I],yM[T]=z[I],t&&(lM[T]=t[I]),T=a[T],s.set(F,T)):z[I]();for(T=d;T<r;T++)T in jM?(j[T]=jM[T],z[T]=yM[T],t&&(t[T]=lM[T],t[T](T))):j[T]=uM(c);j=j.slice(0,u=r),g=i.slice(0)}return j});function c(r){if(z[T]=r,t){const[s,a]=b(T);return t[T]=a,N(i[T],s)}return N(i[T])}}}function e(M,N){return R(()=>M(N))}function TM(){return!0}const ZM={get(M,N,D){return N===jN?D:M.get(N)},has(M,N){return M.has(N)},set:TM,deleteProperty:TM,getOwnPropertyDescriptor(M,N){return{configurable:!0,enumerable:!0,get(){return M.get(N)},set:TM,deleteProperty:TM}},ownKeys(M){return M.keys()}};function CM(M){return typeof M=="function"?M():M}function K(...M){return new Proxy({get(N){for(let D=M.length-1;D>=0;D--){const g=CM(M[D])[N];if(g!==void 0)return g}},has(N){for(let D=M.length-1;D>=0;D--)if(N in CM(M[D]))return!0;return!1},keys(){const N=[];for(let D=0;D<M.length;D++)N.push(...Object.keys(CM(M[D])));return[...new Set(N)]}},ZM)}function lN(M,...N){const D=new Set(N.flat()),g=Object.getOwnPropertyDescriptors(M),j=N.map(z=>{const u={};for(let t=0;t<z.length;t++){const i=z[t];Object.defineProperty(u,i,g[i]?g[i]:{get(){return M[i]},set(){return!0}})}return u});return j.push(new Proxy({get(z){return D.has(z)?void 0:M[z]},has(z){return D.has(z)?!1:z in M},keys(){return Object.keys(M).filter(z=>!D.has(z))}},ZM)),j}function $(M){const N="fallback"in M&&{fallback:()=>M.fallback};return m(yN(()=>M.each,M.children,N||void 0))}function f(M){let N=!1;const D=m(()=>M.when,void 0,{equals:(g,j)=>N?g===j:!g==!j});return m(()=>{const g=D();if(g){const j=M.children;return(N=typeof j=="function"&&j.length>0)?R(()=>j(g)):j}return M.fallback})}const ON=["allowfullscreen","async","autofocus","autoplay","checked","controls","default","disabled","formnovalidate","hidden","indeterminate","ismap","loop","multiple","muted","nomodule","novalidate","open","playsinline","readonly","required","reversed","seamless","selected"],oN=new Set(["className","value","readOnly","formNoValidate","isMap","noModule","playsInline",...ON]),LN=new Set(["innerHTML","textContent","innerText","children"]),xN={className:"class",htmlFor:"for"},VM={class:"className",formnovalidate:"formNoValidate",ismap:"isMap",nomodule:"noModule",playsinline:"playsInline",readonly:"readOnly"},sN=new Set(["beforeinput","click","dblclick","focusin","focusout","input","keydown","keyup","mousedown","mousemove","mouseout","mouseover","mouseup","pointerdown","pointermove","pointerout","pointerover","pointerup","touchend","touchmove","touchstart"]),UN={xlink:"http://www.w3.org/1999/xlink",xml:"http://www.w3.org/XML/1998/namespace"};function EN(M,N){return m(M,void 0,N?void 0:{equals:N})}function aN(M,N,D){let g=D.length,j=N.length,z=g,u=0,t=0,i=N[j-1].nextSibling,I=null;for(;u<j||t<z;){if(N[u]===D[t]){u++,t++;continue}for(;N[j-1]===D[z-1];)j--,z--;if(j===u){const T=z<g?t?D[t-1].nextSibling:D[z-t]:i;for(;t<z;)M.insertBefore(D[t++],T)}else if(z===t)for(;u<j;)(!I||!I.has(N[u]))&&N[u].remove(),u++;else if(N[u]===D[z-1]&&D[t]===N[j-1]){const T=N[--j].nextSibling;M.insertBefore(D[t++],N[u++].nextSibling),M.insertBefore(D[--z],T),N[j]=D[z]}else{if(!I){I=new Map;let c=t;for(;c<z;)I.set(D[c],c++)}const T=I.get(N[u]);if(T!=null)if(t<T&&T<z){let c=u,r=1,s;for(;++c<j&&c<z&&!((s=I.get(N[c]))==null||s!==T+r);)r++;if(r>T-t){const a=N[u];for(;t<T;)M.insertBefore(D[t++],a)}else M.replaceChild(D[t++],N[u++])}else u++;else N[u++].remove()}}}const vM="_$DX_DELEGATE";function CN(M,N,D){let g;return uM(j=>{g=j,N===document?M():n(N,M(),N.firstChild?null:void 0,D)}),()=>{g(),N.textContent=""}}function dM(M,N,D){const g=document.createElement("template");g.innerHTML=M;let j=g.content.firstChild;return D&&(j=j.firstChild),j}function dN(M,N=window.document){const D=N[vM]||(N[vM]=new Set);for(let g=0,j=M.length;g<j;g++){const z=M[g];D.has(z)||(D.add(z),N.addEventListener(z,fN))}}function JM(M,N,D){D==null?M.removeAttribute(N):M.setAttribute(N,D)}function QN(M,N,D,g){g==null?M.removeAttributeNS(N,D):M.setAttributeNS(N,D,g)}function wN(M,N,D,g){g?Array.isArray(D)?(M[`$$${N}`]=D[0],M[`$$${N}Data`]=D[1]):M[`$$${N}`]=D:Array.isArray(D)?M.addEventListener(N,j=>D[0](D[1],j)):M.addEventListener(N,D)}function kN(M,N,D={}){const g=Object.keys(N||{}),j=Object.keys(D);let z,u;for(z=0,u=j.length;z<u;z++){const t=j[z];!t||t==="undefined"||N[t]||(RM(M,t,!1),delete D[t])}for(z=0,u=g.length;z<u;z++){const t=g[z],i=!!N[t];!t||t==="undefined"||D[t]===i||!i||(RM(M,t,!0),D[t]=i)}return D}function SN(M,N,D={}){const g=M.style;if(N==null||typeof N=="string")return g.cssText=N;typeof D=="string"&&(D={});let j,z;for(z in D)N[z]==null&&g.removeProperty(z),delete D[z];for(z in N)j=N[z],j!==D[z]&&(g.setProperty(z,j),D[z]=j);return D}function hN(M,N,D,g){typeof N=="function"?E(j=>PM(M,N(),j,D,g)):PM(M,N,void 0,D,g)}function n(M,N,D,g){if(D!==void 0&&!g&&(g=[]),typeof N!="function")return G(M,N,g,D);E(j=>G(M,N(),j,D),g)}function YN(M,N,D,g,j={}){for(const z in j)if(!(z in N)){if(z==="children")continue;GM(M,z,null,j[z],D)}for(const z in N){if(z==="children"){g||G(M,N.children);continue}const u=N[z];j[z]=GM(M,z,u,j[z],D)}}function pN(M){return M.toLowerCase().replace(/-([a-z])/g,(N,D)=>D.toUpperCase())}function RM(M,N,D){const g=N.trim().split(/\s+/);for(let j=0,z=g.length;j<z;j++)M.classList.toggle(g[j],D)}function GM(M,N,D,g,j){let z,u,t;if(N==="style")return SN(M,D,g);if(N==="classList")return kN(M,D,g);if(D===g)return g;if(N==="ref")D(M);else if(N.slice(0,3)==="on:")M.addEventListener(N.slice(3),D);else if(N.slice(0,10)==="oncapture:")M.addEventListener(N.slice(10),D,!0);else if(N.slice(0,2)==="on"){const i=N.slice(2).toLowerCase(),I=sN.has(i);wN(M,i,D,I),I&&dN([i])}else if((t=LN.has(N))||!j&&(VM[N]||(u=oN.has(N)))||(z=M.nodeName.includes("-")))z&&!u&&!t?M[pN(N)]=D:M[VM[N]||N]=D;else{const i=j&&N.indexOf(":")>-1&&UN[N.split(":")[0]];i?QN(M,i,N,D):JM(M,xN[N]||N,D)}return D}function fN(M){const N=`$$${M.type}`;let D=M.composedPath&&M.composedPath()[0]||M.target;for(M.target!==D&&Object.defineProperty(M,"target",{configurable:!0,value:D}),Object.defineProperty(M,"currentTarget",{configurable:!0,get(){return D||document}});D!==null;){const g=D[N];if(g&&!D.disabled){const j=D[`${N}Data`];if(j!==void 0?g(j,M):g(M),M.cancelBubble)return}D=D.host&&D.host!==D&&D.host instanceof Node?D.host:D.parentNode}}function PM(M,N,D={},g,j){return!j&&"children"in N&&E(()=>D.children=G(M,N.children,D.children)),E(()=>YN(M,N,g,!0,D)),D}function G(M,N,D,g,j){for(;typeof D=="function";)D=D();if(N===D)return D;const z=typeof N,u=g!==void 0;if(M=u&&D[0]&&D[0].parentNode||M,z==="string"||z==="number")if(z==="number"&&(N=N.toString()),u){let t=D[0];t&&t.nodeType===3?t.data=N:t=document.createTextNode(N),D=P(M,D,g,t)}else D!==""&&typeof D=="string"?D=M.firstChild.data=N:D=M.textContent=N;else if(N==null||z==="boolean")D=P(M,D,g);else{if(z==="function")return E(()=>{let t=N();for(;typeof t=="function";)t=t();D=G(M,t,D,g)}),()=>D;if(Array.isArray(N)){const t=[];if(QM(t,N,j))return E(()=>D=G(M,t,D,g,!0)),()=>D;t.length===0?P(M,D,g):Array.isArray(D)?D.length===0?BM(M,t,g):aN(M,D,t):(D&&P(M,D),BM(M,t)),D=t}else if(N instanceof Node){if(Array.isArray(D)){if(u)return D=P(M,D,g,N);P(M,D,null,N)}else D==null||D===""||!M.firstChild?M.appendChild(N):M.replaceChild(N,M.firstChild);D=N}}return D}function QM(M,N,D){let g=!1;for(let j=0,z=N.length;j<z;j++){let u=N[j],t;if(u instanceof Node)M.push(u);else if(!(u==null||u===!0||u===!1))if(Array.isArray(u))g=QM(M,u)||g;else if((t=typeof u)==="string")M.push(document.createTextNode(u));else if(t==="function")if(D){for(;typeof u=="function";)u=u();g=QM(M,Array.isArray(u)?u:[u])||g}else M.push(u),g=!0;else M.push(document.createTextNode(u.toString()))}return g}function BM(M,N,D){for(let g=0,j=N.length;g<j;g++)M.insertBefore(N[g],D)}function P(M,N,D,g){if(D===void 0)return M.textContent="";const j=g||document.createTextNode("");if(N.length){let z=!1;for(let u=N.length-1;u>=0;u--){const t=N[u];if(j!==t){const i=t.parentNode===M;!z&&!u?i?M.replaceChild(j,t):M.insertBefore(j,D):i&&t.remove()}else z=!0}}else M.insertBefore(j,D);return[j]}let bN={data:""},mN=M=>typeof window=="object"?((M?M.querySelector("#_goober"):window._goober)||Object.assign((M||document.head).appendChild(document.createElement("style")),{innerHTML:" ",id:"_goober"})).firstChild:M||bN,$N=/(?:([\u0080-\uFFFF\w-%@]+) *:? *([^{;]+?);|([^;}{]*?) *{)|(}\s*)/g,WN=/\/\*[^]*?\*\/|\s\s+|\n/g,W=(M,N)=>{let D="",g="",j="";for(let z in M){let u=M[z];z[0]=="@"?z[1]=="i"?D=z+" "+u+";":g+=z[1]=="f"?W(u,z):z+"{"+W(u,z[1]=="k"?"":N)+"}":typeof u=="object"?g+=W(u,N?N.replace(/([^,])+/g,t=>z.replace(/(^:.*)|([^,])+/g,i=>/&/.test(i)?i.replace(/&/g,t):t?t+" "+i:i)):z):u!=null&&(z=z.replace(/[A-Z]/g,"-$&").toLowerCase(),j+=W.p?W.p(z,u):z+":"+u+";")}return D+(N&&j?N+"{"+j+"}":j)+g},q={},FM=M=>{if(typeof M=="object"){let N="";for(let D in M)N+=D+FM(M[D]);return N}return M},ZN=(M,N,D,g,j)=>{let z=FM(M),u=q[z]||(q[z]=(t=>{let i=0,I=11;for(;i<t.length;)I=101*I+t.charCodeAt(i++)>>>0;return"go"+I})(z));if(!q[u]){let t=z!==M?M:(i=>{let I,T=[{}];for(;I=$N.exec(i.replace(WN,""));)I[4]?T.shift():I[3]?T.unshift(T[0][I[3]]=T[0][I[3]]||{}):T[0][I[1]]=I[2];return T[0]})(M);q[u]=W(j?{["@keyframes "+u]:t}:t,D?"":"."+u)}return((t,i,I)=>{i.data.indexOf(t)==-1&&(i.data=I?t+i.data:i.data+t)})(q[u],N,g),u},VN=(M,N,D)=>M.reduce((g,j,z)=>{let u=N[z];if(u&&u.call){let t=u(D),i=t&&t.props&&t.props.className||/^go/.test(t)&&t;u=i?"."+i:t&&typeof t=="object"?t.props?"":W(t,""):t===!1?"":t}return g+j+(u??"")},"");function iM(M){let N=this||{},D=M.call?M(N.p):M;return ZN(D.unshift?D.raw?VN(D,[].slice.call(arguments,1),N.p):D.reduce((g,j)=>Object.assign(g,j&&j.call?j(N.p):j),{}):D,mN(N.target),N.g,N.o,N.k)}iM.bind({g:1}),iM.bind({k:1});const XM=TN();function vN(M){return e(XM.Provider,{value:M.theme,get children(){return M.children}})}function A(M){let N=this||{};return(...D)=>{const g=j=>{const z=iN(XM),u=K(j,{theme:z}),t=K(u,{get className(){const r=u.className,s="className"in u&&/^go[0-9]+/.test(r);let a=iM.apply({target:N.target,o:s,p:u,g:N.g},D);return[r,a].filter(Boolean).join(" ")}}),[i,I]=lN(t,["as"]),T=i.as||M;let c;return typeof T=="function"?c=T(I):(c=document.createElement(T),hN(c,I)),c};return g.className=j=>R(()=>iM.apply({target:N.target,p:j,g:N.g},D)),g}}function JN(){const M=A.call({g:1},"div").apply(null,arguments);return function(D){return M(D),null}}function RN(M,N){return m(M,void 0,N?void 0:{equals:N})}function _M(M,N,D){let g=D.length,j=N.length,z=g,u=0,t=0,i=N[j-1].nextSibling,I=null;for(;u<j||t<z;){if(N[u]===D[t]){u++,t++;continue}for(;N[j-1]===D[z-1];)j--,z--;if(j===u){const T=z<g?t?D[t-1].nextSibling:D[z-t]:i;for(;t<z;)M.insertBefore(D[t++],T)}else if(z===t)for(;u<j;)(!I||!I.has(N[u]))&&N[u].remove(),u++;else if(N[u]===D[z-1]&&D[t]===N[j-1]){const T=N[--j].nextSibling;M.insertBefore(D[t++],N[u++].nextSibling),M.insertBefore(D[--z],T),N[j]=D[z]}else{if(!I){I=new Map;let c=t;for(;c<z;)I.set(D[c],c++)}const T=I.get(N[u]);if(T!=null)if(t<T&&T<z){let c=u,r=1,s;for(;++c<j&&c<z&&!((s=I.get(N[c]))==null||s!==T+r);)r++;if(r>T-t){const a=N[u];for(;t<T;)M.insertBefore(D[t++],a)}else M.replaceChild(D[t++],N[u++])}else u++;else N[u++].remove()}}}const HM="_$DX_DELEGATE";function L(M,N,D){const g=document.createElement("template");g.innerHTML=M;let j=g.content.firstChild;return D&&(j=j.firstChild),j}function GN(M,N=window.document){const D=N[HM]||(N[HM]=new Set);for(let g=0,j=M.length;g<j;g++){const z=M[g];D.has(z)||(D.add(z),N.addEventListener(z,PN))}}function w(M,N,D){D==null?M.removeAttribute(N):M.setAttribute(N,D)}function KM(M,N,D,g){if(D!==void 0&&!g&&(g=[]),typeof N!="function")return IM(M,N,g,D);E(j=>IM(M,N(),j,D),g)}function PN(M){const N=`$$${M.type}`;let D=M.composedPath&&M.composedPath()[0]||M.target;for(M.target!==D&&Object.defineProperty(M,"target",{configurable:!0,value:D}),Object.defineProperty(M,"currentTarget",{configurable:!0,get(){return D||document}});D!==null;){const g=D[N];if(g&&!D.disabled){const j=D[`${N}Data`];if(j!==void 0?g(j,M):g(M),M.cancelBubble)return}D=D.host&&D.host!==D&&D.host instanceof Node?D.host:D.parentNode}}function IM(M,N,D,g,j){for(;typeof D=="function";)D=D();if(N===D)return D;const z=typeof N,u=g!==void 0;if(M=u&&D[0]&&D[0].parentNode||M,z==="string"||z==="number")if(z==="number"&&(N=N.toString()),u){let t=D[0];t&&t.nodeType===3?t.data=N:t=document.createTextNode(N),D=MM(M,D,g,t)}else D!==""&&typeof D=="string"?D=M.firstChild.data=N:D=M.textContent=N;else if(N==null||z==="boolean")D=MM(M,D,g);else{if(z==="function")return E(()=>{let t=N();for(;typeof t=="function";)t=t();D=IM(M,t,D,g)}),()=>D;if(Array.isArray(N)){const t=[];if(wM(t,N,j))return E(()=>D=IM(M,t,D,g,!0)),()=>D;if(t.length===0){if(D=MM(M,D,g),u)return D}else Array.isArray(D)?D.length===0?qM(M,t,g):_M(M,D,t):D==null||D===""?qM(M,t):_M(M,u&&D||[M.firstChild],t);D=t}else if(N instanceof Node){if(Array.isArray(D)){if(u)return D=MM(M,D,g,N);MM(M,D,null,N)}else D==null||D===""||!M.firstChild?M.appendChild(N):M.replaceChild(N,M.firstChild);D=N}}return D}function wM(M,N,D){let g=!1;for(let j=0,z=N.length;j<z;j++){let u=N[j],t;if(u instanceof Node)M.push(u);else if(!(u==null||u===!0||u===!1))if(Array.isArray(u))g=wM(M,u)||g;else if((t=typeof u)==="string")M.push(document.createTextNode(u));else if(t==="function")if(D){for(;typeof u=="function";)u=u();g=wM(M,Array.isArray(u)?u:[u])||g}else M.push(u),g=!0;else M.push(document.createTextNode(u.toString()))}return g}function qM(M,N,D){for(let g=0,j=N.length;g<j;g++)M.insertBefore(N[g],D)}function MM(M,N,D,g){if(D===void 0)return M.textContent="";const j=g||document.createTextNode("");if(N.length){let z=!1;for(let u=N.length-1;u>=0;u--){const t=N[u];if(j!==t){const i=t.parentNode===M;!z&&!u?i?M.replaceChild(j,t):M.insertBefore(j,D):i&&t.remove()}else z=!0}}else M.insertBefore(j,D);return[j]}const BN=L('<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M8 0C7.44772 0 7 0.447715 7 1V7H1C0.447715 7 0 7.44772 0 8C0 8.55228 0.447715 9 1 9H7V15C7 15.5523 7.44772 16 8 16C8.55228 16 9 15.5523 9 15V9H15C15.5523 9 16 8.55228 16 8C16 7.44772 15.5523 7 15 7H9V1C9 0.447715 8.55228 0 8 0Z"></path></svg>'),FN=L('<svg width="16" height="12" viewBox="0 0 16 12" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M0 1C0 0.447715 0.447715 0 1 0H15C15.5523 0 16 0.447715 16 1C16 1.55228 15.5523 2 15 2H1C0.447715 2 0 1.55228 0 1ZM0 6C0 5.44772 0.447715 5 1 5H15C15.5523 5 16 5.44772 16 6C16 6.55228 15.5523 7 15 7H1C0.447715 7 0 6.55228 0 6ZM1 10C0.447715 10 0 10.4477 0 11C0 11.5523 0.447715 12 1 12H15C15.5523 12 16 11.5523 16 11C16 10.4477 15.5523 10 15 10H1Z"></path></svg>'),XN=L('<svg width="14" height="16" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M0.292893 0.292893C-0.0976311 0.683418 -0.0976311 1.31658 0.292893 1.70711L5.58579 7L0.292893 12.2929C-0.0976309 12.6834 -0.0976309 13.3166 0.292893 13.7071C0.683418 14.0976 1.31658 14.0976 1.70711 13.7071L7 8.41421L12.2929 13.7071C12.6834 14.0976 13.3166 14.0976 13.7071 13.7071C14.0976 13.3166 14.0976 12.6834 13.7071 12.2929L8.41421 7L13.7071 1.70711C14.0976 1.31658 14.0976 0.683418 13.7071 0.292893C13.3166 -0.0976311 12.6834 -0.0976311 12.2929 0.292893L7 5.58579L1.70711 0.292893C1.31658 -0.0976311 0.683418 -0.0976311 0.292893 0.292893Z"></path></svg>'),_N=L('<svg width="4" height="16" viewBox="0 0 4 18" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M2 4C3.10457 4 4 3.10457 4 2C4 0.89543 3.10457 0 2 0C0.89543 0 0 0.89543 0 2C0 3.10457 0.89543 4 2 4ZM2 11C3.10457 11 4 10.1046 4 9C4 7.89543 3.10457 7 2 7C0.89543 7 0 7.89543 0 9C0 10.1046 0.89543 11 2 11ZM4 16C4 17.1046 3.10457 18 2 18C0.89543 18 0 17.1046 0 16C0 14.8954 0.89543 14 2 14C3.10457 14 4 14.8954 4 16Z"></path></svg>'),HN=L('<svg width="16" height="16" viewBox="0 0 16 2" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1 2C0.447715 2 0 1.55228 0 1C0 0.447715 0.447715 0 1 0H15C15.5523 0 16 0.447715 16 1C16 1.55228 15.5523 2 15 2H1Z"></path></svg>'),KN=L('<svg width="17" height="16" viewBox="0 0 17 19" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M7.5 15C3.35786 15 0 11.6421 0 7.5C0 3.35786 3.35786 0 7.5 0C11.6421 0 15 3.35786 15 7.5C15 9.48047 14.2324 11.2816 12.9784 12.6222L16.7809 17.3753C17.1259 17.8066 17.056 18.4359 16.6247 18.7809C16.1934 19.1259 15.5641 19.056 15.2191 18.6247L11.4304 13.8888C10.2875 14.5935 8.94124 15 7.5 15ZM7.5 13C10.5376 13 13 10.5376 13 7.5C13 4.46243 10.5376 2 7.5 2C4.46243 2 2 4.46243 2 7.5C2 10.5376 4.46243 13 7.5 13Z"></path></svg>'),qN=L('<svg width="16" height="16" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M6 12C9.31371 12 12 9.31371 12 6C12 2.68629 9.31371 0 6 0C2.68629 0 0 2.68629 0 6C0 9.31371 2.68629 12 6 12Z"></path></svg>'),MD=L('<svg width="16" height="16" viewBox="0 0 9 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M2.41421 8L8.70711 14.2929C9.09763 14.6834 9.09763 15.3166 8.70711 15.7071C8.31658 16.0976 7.68342 16.0976 7.29289 15.7071L0.292893 8.70711C-0.0976311 8.31658 -0.0976311 7.68342 0.292893 7.29289L7.29289 0.292893C7.68342 -0.0976311 8.31658 -0.0976311 8.70711 0.292893C9.09763 0.683418 9.09763 1.31658 8.70711 1.70711L2.41421 8Z"></path></svg>'),ND=L('<svg width="16" height="9" viewBox="0 0 16 9" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M8 6.58579L14.2929 0.292893C14.6834 -0.0976311 15.3166 -0.0976311 15.7071 0.292893C16.0976 0.683418 16.0976 1.31658 15.7071 1.70711L8.70711 8.70711C8.31658 9.09763 7.68342 9.09763 7.29289 8.70711L0.292893 1.70711C-0.0976311 1.31658 -0.0976311 0.683418 0.292893 0.292893C0.683418 -0.0976311 1.31658 -0.0976311 1.70711 0.292893L8 6.58579Z"></path></svg>'),k={Plus:({fill:M})=>(()=>{const N=BN.cloneNode(!0),D=N.firstChild;return w(D,"fill",M),N})(),Burger:({fill:M})=>(()=>{const N=FN.cloneNode(!0),D=N.firstChild;return w(D,"fill",M),N})(),Cross:({fill:M})=>(()=>{const N=XN.cloneNode(!0),D=N.firstChild;return w(D,"fill",M),N})(),More:({fill:M})=>(()=>{const N=_N.cloneNode(!0),D=N.firstChild;return w(D,"fill",M),N})(),Minus:({fill:M})=>(()=>{const N=HN.cloneNode(!0),D=N.firstChild;return w(D,"fill",M),N})(),Lens:({fill:M})=>(()=>{const N=KN.cloneNode(!0),D=N.firstChild;return w(D,"fill",M),N})(),Circle:({fill:M})=>(()=>{const N=qN.cloneNode(!0),D=N.firstChild;return w(D,"fill",M),N})(),ChevronLeft:({fill:M})=>(()=>{const N=MD.cloneNode(!0),D=N.firstChild;return w(D,"fill",M),N})(),ChevronDown:({fill:M})=>(()=>{const N=ND.cloneNode(!0),D=N.firstChild;return w(D,"fill",M),N})()},S=A("span")`
	height: 20px;
	width: 20px;
	display: flex;
	justify-content: center;
	align-items: center;
`,Z=Object.assign({},{Plus:({fill:M="#2c2738",onClick:N})=>e(S,{onClick:N,get children(){return e(k.Plus,{fill:M})}}),Cross:({fill:M="#2c2738",onClick:N})=>e(S,{onClick:N,get children(){return e(k.Cross,{fill:M})}}),Minus:({fill:M="#2c2738",onClick:N})=>e(S,{onClick:N,get children(){return e(k.Minus,{fill:M})}}),More:({fill:M="#2c2738",onClick:N})=>e(S,{onClick:N,get children(){return e(k.More,{fill:M})}}),Burger:({fill:M="#2c2738",onClick:N})=>e(S,{onClick:N,get children(){return e(k.Burger,{fill:M})}}),Lens:({fill:M="#2c2738",onClick:N})=>e(S,{onClick:N,get children(){return e(k.Lens,{fill:M})}}),Circle:({fill:M="#2c2738",onClick:N})=>e(S,{onClick:N,get children(){return e(k.Circle,{fill:M})}}),ChevronLeft:({fill:M="#2c2738",onClick:N})=>e(S,{onClick:N,get children(){return e(k.ChevronLeft,{fill:M})}}),ChevronDown:({fill:M="#2c2738",onClick:N})=>e(S,{onClick:N,get children(){return e(k.ChevronDown,{fill:M})}})}),DD=M=>{switch(M){case 1:return"72px";case 2:return"64px";case 3:return"56px";case 4:return"34px";case 5:return"28px";case 6:return"20px";default:return"20px"}},eD=A("h1")`
	font-size: ${M=>DD(M.size)};
	font-weight: ${M=>M.weight};
  color: ${M=>M.theme.colors[M.type]};
`,V=({size:M=1,type:N="primary",weight:D="normal",children:g})=>e(eD,{size:M,weight:D,type:N,children:g}),gD=M=>{switch(M){case 1:return"16px";case 2:return"14px";default:return"16px"}},jD=A("p")`
	font-size: ${M=>gD(M.size)};
	font-weight: ${M=>M.weight};
  color: ${M=>M.theme.colors[M.type]};
`,NM=({size:M=1,weight:N="normal",type:D="primary",children:g})=>e(jD,{size:M,weight:N,type:D,children:g}),MN=({type:M="primary",children:N})=>e(NM,{size:1,type:M,weight:"normal",children:N}),{Cross:zD}=Z,tD=A("div")`
	background-color: ${M=>M.theme.colors[M.type]};
	box-sizing: border-box;
  box-shadow: rgba(99, 99, 99, 0.2) 0px 2px 8px 0px;
  width: 100%;
  display: flex;
  justify-content: space-between;
  align-items: center;
  padding: 15px 24px;
  border-radius: 10px;
	color: ${M=>M.theme.colors[M.textColor]};
  font-weight: 400;
	gap: 8px;

	& svg {
		cursor: pointer;
	}
`,v=({type:M="bright",textColor:N="bright",iconColor:D="#ffffff",children:g})=>{const[j,z]=b(!1);return e(f,{get when(){return!j()},get children(){return e(tD,{type:M,textColor:N,get children(){return[e(NM,{type:N,children:g}),e(zD,{fill:D,onClick:()=>z(!0)})]}})}})},uD=A("div")`
	height: 56px;
  width: 56px;
  display: flex;
  border-radius: ${M=>M.round?"50%":"4px"};
  justify-content: center;
  align-items: center;
  font-size: 16px;
  background: ${M=>M.theme.colors.muted};
  color: ${M=>M.theme.colors.bright};
  font-weight: bold;
`,TD=({initials:M,round:N=!1})=>e(uD,{round:N,children:M}),iD=M=>`https://storage.googleapis.com/rev-kit-assets/${M}.png`,ID=A("div")`
	height: 56px;
	width: 56px;
	border-radius: ${M=>M.round?"50%":"4px"};
	background-size: cover;
	background-image: ${M=>`url(${iD(M.type)})`};
`,AM=({type:M="steven",round:N=!1})=>e(ID,{type:M,round:N}),C=Object.assign(TD,{Steven:({round:M})=>e(AM,{type:"steven",round:M}),Mike:({round:M})=>e(AM,{type:"mike",round:M}),Mili:({round:M})=>e(AM,{type:"mili",round:M}),Meg:({round:M})=>e(AM,{type:"meg",round:M})}),AD=A("button")`
  box-sizing: border-box;
  border: unset;
  border-radius: 3px;
  height: ${M=>M.small?"34px":"48px"};
  padding: 4px 20px;
  font-size: 14px;
  min-width: 100px;

	&.bright {
			background: ${M=>M.theme.colors.bright};
  		color: ${M=>M.theme.colors.primary};
  		box-shadow: rgba(99, 99, 99, 0.2) 0px 2px 8px 0px;

			&:hover {
				box-shadow: rgba(0, 0, 0, 0.1) 0px 4px 12px;
			}

			&:active {
				border: 2px solid ${M=>M.theme.colors.primary};
  			box-shadow: unset;
			}

			&:disabled {
				background: ${M=>M.theme.colors.shade};
  			color: rgba(44, 39, 56, 0.24);
  			box-shadow: unset;
			}
	}

	&.accent {
		background: ${M=>M.theme.colors.accent};
		color: ${M=>M.theme.colors.bright};

		&:hover {
			box-shadow: rgba(0, 0, 0, 0.1) 0px 4px 12px;
		}

		&:active {
			border: 2px solid ${M=>M.theme.colors.primary};
			box-shadow: unset;
		}

		&:disabled {
			background: ${M=>M.theme.colors.shade};
			color: rgba(44, 39, 56, 0.24);
			box-shadow: unset;
		}
	}

	&.ghost {
		background: ${M=>M.theme.colors.bright};
		color: ${M=>M.theme.colors.muted};
		border: 2px solid ${M=>M.theme.colors.muted};
		box-shadow: unset;

		&:hover {
			color: ${M=>M.theme.colors.accent};
  		border-color: ${M=>M.theme.colors.accent};
  		box-shadow: unset;
		}

		&:active {
			color: ${M=>M.theme.colors.secondary};
  		border-color: ${M=>M.theme.colors.secondary};
		}

		&:disabled {
			background: transparent;
  		color: rgba(44, 39, 56, 0.24);
  		border-color: rgba(44, 39, 56, 0.24);
		}
	}
`,y=({variant:M="accent",disabled:N=!1,small:D=!1,onClick:g,children:j})=>e(AD,{variant:M,onClick:g,small:D,disabled:N,className:`${M}`,children:j}),cD=A("div")`
	width: 100%;
	height: 80%;
	display: inline-flex;
	flex-wrap: wrap;
	justify-content: space-between;
	align-items: center;
	background: ${M=>M.theme.colors.bright};
	color: ${M=>M.theme.colors.primary};
	padding: 24px 20px;
	border-radius: 8px;
	box-shadow: rgba(0, 0, 0, 0.02) 0px 1px 3px 0px, rgba(27, 31, 35, 0.15) 0px 0px 0px 1px;
`,NN=A("div")`
	display: inline-flex;
	justify-content: ${M=>M.small?"flex-end":"flex-start"};
	align-items: center;
	gap: 8px;
`,nD=A("div")`
	width: 100%;
	height: auto;
	min-height: 200px;
	padding: 40px;
	display: flex;
	flex-direction: column;
	background: ${M=>M.theme.colors.bright};
	color: ${M=>M.theme.colors.primary};
	box-shadow: rgba(0, 0, 0, 0.02) 0px 1px 3px 0px, rgba(27, 31, 35, 0.15) 0px 0px 0px 1px;
	gap: 16px;
	border-radius: 16px;
`,rD=({text:M,actions:N})=>e(cD,{get children(){return[e(V,{size:6,children:M}),e(NN,{small:!0,get children(){return e($,{each:N,children:D=>D})}})]}}),DN=({title:M,text:N,actions:D,small:g=!1})=>e(f,{when:!g,fallback:()=>e(rD,{text:N,actions:D}),get children(){return e(nD,{get children(){return[e(V,{size:4,children:M}),e(NM,{children:N}),e(NN,{small:g,get children(){return e($,{each:D,children:j=>j})}})]}})}}),yD=A("div")`
	height: fit-content;
  width: 300px;
  padding: 16px;
  box-sizing: border-box;
  display: flex;
  flex-direction: column;
  justify-content: flex-start;
  box-shadow: rgba(0, 0, 0, 0.1) 0px 4px 12px;
  border-radius: 24px;
	background: ${M=>M.theme.colors.bright};
	gap: 8px;
`,lD=A("div")`
	height: 200px;
	background: ${M=>M.src?`url(${M.src})`:"unset"};
	background-size: cover;
  border-radius: 16px;
  width: 100%;
`,OD=A("div")`
  padding: 8px 0;
  height: auto;
  font-size: 14px;
`,oD=A("div")`
  height: auto;
  font-size: 14px;
  padding: 8px 0;
`,eN=({imageSrc:M,title:N,children:D,actions:g})=>e(yD,{get children(){return[e(f,{when:M,get children(){return e(lD,{src:M})}}),e(V,{size:5,weight:"bold",children:N}),e(oD,{children:D}),e(OD,{get children(){return e($,{each:g,children:j=>j})}})]}});L("<label></label>"),L("<h2></h2>"),L("<p></p>"),A("div")`
	background-color: ${M=>M.backgroundColor};
	color: ${M=>M.color};
	height: 240px;
	width: 260px;
	border-radius: 20px;
	padding: 16px 20px;
	box-shadow: rgba(99, 99, 99, 0.2) 0px 2px 8px 0px;
`,A("div")`
	display: inline-flex;
	justify-content: flex-end;
	width: 100%;
	height: 60%;
`;const LD=A("div")`
	display: inline-flex;
	align-items: center;
	height: 52px;
	background: ${M=>M.disabled?M.theme.colors.shade:M.theme.colors.bright};
	border-radius: 6px;
`,gN=A("button")`
	display: inline-flex;
	justify-content: center;
	align-items: center;
	padding: 12px;
	width: 60px;
	background: transparent;
	border: unset;
	outline: unset;
	height: 100%;
	cursor: pointer;

	${M=>M.side==="left"?`
			border-top-left-radius: 6px;
			border-bottom-left-radius: 6px;
		`:`
			border-top-right-radius: 6px;
			border-bottom-right-radius: 6px;
		`}

	&:active {
		background: ${M=>M.theme.colors.accent};

		& > span > svg > path {
			fill: ${M=>M.theme.colors.bright};
		}
	}

	&:disabled {
		background: ${M=>M.theme.colors.shade};
		& > span > svg > path {
			fill: ${M=>M.theme.colors.secondary};
		}
	}
`,xD=A("input")`
	width: 60px;
	padding: 12px;
	outline: unset;
	border: unset;
	text-align: center;
	font-size: 16px;
	height: 100%;
	border-left: 1px solid ${M=>M.theme.colors.shade};
	border-right: 1px solid ${M=>M.theme.colors.shade};
	background: transparent;
`,kM=({defaultValue:M=0,disabled:N,maxValue:D=999,minValue:g=-999,onInput:j,...z})=>{const[u,t]=b(M),i=c=>{/^(0|-*[1-9]+[0-9]*)$/.test(c?.target?.value)||(c.target.value=c.target.value.slice(0,-1)),t(Number(c.target.value)??0),j?.(c)},I=()=>t(c=>c+1),T=()=>t(c=>c-1);return e(LD,{disabled:N,get children(){return[e(gN,{onClick:T,side:"left",get disabled(){return N||u()===g},get children(){return e(Z.Minus,{})}}),e(xD,K({get value(){return u()},onInput:i,disabled:N},z)),e(gN,{onClick:I,side:"right",get disabled(){return N||u()===D},get children(){return e(Z.Plus,{})}})]}})},sD=A("div")`
	display: inline-flex;
	justify-content: space-between;
	align-items: center;
	height: 52px;
	outline: unset;
	border-radius: 6px;
	background: ${M=>M.disabled?M.theme.colors.shade:M.theme.colors.bright};
	border: 1px solid ${M=>M.theme.colors.shade};
	font-size: 16px;
	box-sizing: border-box;
	gap: 16px;
	padding: 0 16px;
	min-width: 360px;

	&:focus-within {
		outline: none;
		border: 2px solid ${M=>M.theme.colors.accent};
	}
`,UD=A("input")`
	outline: unset;
	background: transparent;
	border: unset;
	font-size: 16px;
	margin: 16px 0;

	&::placeholder {
		color: ${M=>M.theme.colors.muted};
	}

	&:disabled, &:disabled::placeholder {
		color: ${M=>M.theme.colors.secondary};
	}
`,DM=({icon:M,disabled:N,...D})=>e(sD,{disabled:N,get children(){return[e(UD,K({disabled:N},D)),e(f,{when:M,children:M})]}}),ED=A("textarea")`
	outline: unset;
	background: ${M=>M.theme.colors.bright};
	border: 1px solid ${M=>M.theme.colors.shade};
	font-size: 16px;
	padding: 16px;
	border-radius: 6px;
	height: fit-content;
	min-width: 360px;

	&:focus {
		outline: unset;
		border: 2px solid ${M=>M.theme.colors.accent};
	}

	&::placeholder {
		color: ${M=>M.theme.colors.muted};
	}

	&:disabled, &:disabled::placeholder {
		color: ${M=>M.theme.colors.secondary};
		background: ${M=>M.theme.colors.shade};
	}
`,eM=({rows:M=4,...N})=>e(ED,K({rows:M},N)),aD=A("div")`
	display: inline-flex;
  gap: 8px;
`,CD=({children:M})=>e(aD,{children:M}),{Cross:dD}=Z,QD=A("div")`
  position: fixed;
  top: 0;
  right: 0;
  bottom: 0;
  left: 0;
  z-index: 5;
  overflow: auto;
  outline: 0;
`,wD=A("div")`
	box-sizing: border-box;
  background: ${M=>M.theme.colors.bright};
  color: ${M=>M.theme.colors.primary};
  font-size: 14px;
  line-height: 1.5;
  position: relative;
  top: 100px;
  z-index: 6;
  max-width: 500px;
  width: auto;
  margin: 0 auto;
  border-radius: 16px;
  box-shadow: rgba(0, 0, 0, 0.24) 0px 3px 8px;
  padding: 16px 24px;
`,kD=A("div")`
	display: inline-flex;
  justify-content: space-between;
  align-items: center;
  width: 100%;

	& svg {
		cursor: pointer;
	}
`,SD=A("div")`
	padding: 8px 0;
`,hD=A("div")`
	width: 100%;
  display: inline-flex;
  justify-content: flex-end;
  align-items: center;
`,YD=({visible:M,title:N,onCancel:D,onOk:g,children:j})=>e(f,{get when(){return M()},get children(){return e(QD,{get children(){return e(wD,{get children(){return[e(kD,{get children(){return[e(V,{size:5,weight:"bold",children:N}),e(dD,{onClick:D})]}}),e(SD,{children:j}),e(hD,{get children(){return e(CD,{get children(){return[e(y,{variant:"ghost",onClick:D,children:"Cancel"}),e(y,{onClick:g,children:"Action"})]}})}})]}})}})}}),pD=L('<div class="progress"></div>'),fD=A("div")`
	width: 100%;
	height: 8px;
	background: ${M=>M.theme.colors.shade};
	border-radius: 2px;

	.progress {
		background: ${M=>M.theme.colors[M.type]};
		width: ${M=>`${M.percent}%`};
		height: 8px;
		border-radius: 2px;
		
		${M=>M.loading?`
			animation-name: loading;
  		animation-duration: 4s;
			animation-iteration-count: infinite;
		`:""};
	}

	@keyframes loading {
		from {width: 0%;}
		to {width: 100%;}
	}
`,gM=({type:M="accent",percent:N,loading:D=!1})=>e(fD,{type:M,percent:N,loading:D,get children(){return pD.cloneNode(!0)}}),bD=JN`
	@import url('https://fonts.googleapis.com/css2?family=IBM+Plex+Sans');

	*,
	*::after,
	*::before {
		box-sizing: border-box;
		margin: 0;
		padding: 0;
	}
	
	* {
		font-family: 'IBM Plex Sans', sans-serif;
	}

	body {
		background-color: #ebf4f8;
	}

	.tippy-box[data-animation=fade][data-state=hidden] {
		opacity: 0
	}

	[data-tippy-root] {
		max-width: calc(100vw - 10px)
	}

	.tippy-box {
		position: relative;
		background-color: #333;
		color: #fff;
		border-radius: 4px;
		font-size: 14px;
		line-height: 1.4;
		white-space: normal;
		outline: 0;
		transition-property: transform, visibility, opacity
	}

	.tippy-box[data-placement^=top]>.tippy-arrow {
		bottom: 0
	}

	.tippy-box[data-placement^=top]>.tippy-arrow:before {
		bottom: -7px;
		left: 0;
		border-width: 8px 8px 0;
		border-top-color: initial;
		transform-origin: center top
	}

	.tippy-box[data-placement^=bottom]>.tippy-arrow {
		top: 0
	}

	.tippy-box[data-placement^=bottom]>.tippy-arrow:before {
		top: -7px;
		left: 0;
		border-width: 0 8px 8px;
		border-bottom-color: initial;
		transform-origin: center bottom
	}

	.tippy-box[data-placement^=left]>.tippy-arrow {
		right: 0
	}

	.tippy-box[data-placement^=left]>.tippy-arrow:before {
		border-width: 8px 0 8px 8px;
		border-left-color: initial;
		right: -7px;
		transform-origin: center left
	}

	.tippy-box[data-placement^=right]>.tippy-arrow {
		left: 0
	}

	.tippy-box[data-placement^=right]>.tippy-arrow:before {
		left: -7px;
		border-width: 8px 8px 8px 0;
		border-right-color: initial;
		transform-origin: center right
	}

	.tippy-box[data-inertia][data-state=visible] {
		transition-timing-function: cubic-bezier(.54, 1.5, .38, 1.11)
	}

	.tippy-arrow {
		width: 16px;
		height: 16px;
		color: #333
	}

	.tippy-arrow:before {
		content: "";
		position: absolute;
		border-color: transparent;
		border-style: solid
	}

	.tippy-content {
		position: relative;
		padding: 5px 9px;
		z-index: 1
	}

	.tippy-box[data-theme~='accent'] {
		background-color: ${M=>M.theme.colors.accent};
		color: ${M=>M.theme.colors.bright};
		padding: 7px 10px;
		border-radius: 4px;
	}

	.tippy-box[data-theme~='accent'][data-placement^='top'] > .tippy-arrow::before {
  	border-top-color: ${M=>M.theme.colors.accent};
	}

	.tippy-box[data-theme~='accent'][data-placement^='bottom'] > .tippy-arrow::before {
		border-bottom-color: ${M=>M.theme.colors.accent};
	}

	.tippy-box[data-theme~='accent'][data-placement^='left'] > .tippy-arrow::before {
		border-left-color: ${M=>M.theme.colors.accent};
	}

	.tippy-box[data-theme~='accent'][data-placement^='right'] > .tippy-arrow::before {
		border-right-color: ${M=>M.theme.colors.accent};
	}

	.tippy-box[data-theme~='primary'] {
		background-color: ${M=>M.theme.colors.primary};
		color: ${M=>M.theme.colors.bright};
		padding: 7px 10px;
		border-radius: 4px;
	}

	.tippy-box[data-theme~='primary'][data-placement^='top'] > .tippy-arrow::before {
  	border-top-color: ${M=>M.theme.colors.primary};
	}

	.tippy-box[data-theme~='primary'][data-placement^='bottom'] > .tippy-arrow::before {
		border-bottom-color: ${M=>M.theme.colors.primary};
	}

	.tippy-box[data-theme~='primary'][data-placement^='left'] > .tippy-arrow::before {
		border-left-color: ${M=>M.theme.colors.primary};
	}

	.tippy-box[data-theme~='primary'][data-placement^='right'] > .tippy-arrow::before {
		border-right-color: ${M=>M.theme.colors.primary};
	}

	.tippy-box[data-theme~='bright'] {
		background-color: ${M=>M.theme.colors.bright};
		color: ${M=>M.theme.colors.primary};
		padding: 7px 10px;
		border-radius: 4px;
		box-shadow: rgba(99, 99, 99, 0.2) 0px 2px 8px 0px;
	}

	.tippy-box[data-theme~='bright'][data-placement^='top'] > .tippy-arrow::before {
  	border-top-color: ${M=>M.theme.colors.bright};
	}

	.tippy-box[data-theme~='bright'][data-placement^='bottom'] > .tippy-arrow::before {
		border-bottom-color: ${M=>M.theme.colors.bright};
	}

	.tippy-box[data-theme~='bright'][data-placement^='left'] > .tippy-arrow::before {
		border-left-color: ${M=>M.theme.colors.bright};
	}

	.tippy-box[data-theme~='bright'][data-placement^='right'] > .tippy-arrow::before {
		border-right-color: ${M=>M.theme.colors.bright};
	}
`,mD={colors:{accent:"#0880AE",warning:"#F2AC57",success:"#14A38B",error:"#FF7171",primary:"#2C2738",secondary:"#756F86",muted:"#7C9CBF",bright:"#FFFFFF",shade:"#DBE2EA",tint:"#EBF4F8"}},$D=M=>e(vN,{theme:mD,get children(){return[e(bD,{}),RN(()=>M.children)]}}),WD=L('<div class="select"></div>'),{ChevronLeft:ZD,ChevronDown:VD}=Z,vD=A("div")`
	position: relative;
	user-select: none;
	outline: none;
	width: auto;
	height: auto;

	& .select {
		background: ${M=>M.theme.colors.bright};
    border: 1px solid ${M=>M.theme.colors.shade};
		border-radius: 6px;
    display: inline-flex;
    justify-content: space-between;
    align-items: center;
    margin: 0;
		min-width: 360px;
    height: 52px;
    padding: 5px;
    box-sizing: border-box;
		padding: 16px;

		& span svg path {
			fill: ${M=>M.theme.colors.accent};
		}

		&.selected {
			border: 2px solid ${M=>M.theme.colors.accent};
		}

		&.disabled {
			background: ${M=>M.theme.colors.shade};
			color: ${M=>M.theme.colors.secondary};

			& span svg path {
				fill: ${M=>M.theme.colors.secondary};
			}
		}
	}
`,JD=A("span")`
	color: ${M=>M.theme.colors.muted};
`,RD=A("div")`
	position: absolute;
	top: 60px;
	display: flex;
	flex-direction: column;
	min-width: 360px;
	list-style-type: none;
	padding: 12px 0;
	border-radius: 6px;
	background: ${M=>M.theme.colors.bright};
	z-index: 3;
`,GD=A("div")`
	height: 44px;
	text-align: left;
	padding: 12px 15px;
	background: ${M=>M.selected?M.theme.colors.tint:M.theme.colors.bright};

	&:hover, &.selected  {
		background: ${M=>M.theme.colors.tint};
	}
`,PD=(M,N)=>{const D=g=>!M.contains(g.target)&&N()?.();document.body.addEventListener("click",D),hM(()=>document.body.removeEventListener("click",D))},B=({options:M=["test"],placeholder:N="Select",defaultOption:D,disabled:g=!1})=>{const[j,z]=b(!1),[u,t]=b(D),i=T=>{t(T),z(!1)},I=()=>{g||z(T=>!T)};return e(vD,{get children(){return[(()=>{const T=WD.cloneNode(!0);return PD(T,()=>()=>z(!1)),T.$$click=I,T.classList.toggle("disabled",g),KM(T,e(f,{get when(){return u()},fallback:()=>e(JD,{children:N}),get children(){return u()}}),null),KM(T,e(f,{get when(){return j()},fallback:()=>e(ZD,{}),get children(){return e(VD,{})}}),null),E(()=>T.classList.toggle("selected",j())),T})(),e(f,{get when(){return j()},get children(){return e(RD,{get children(){return e($,{each:M,children:T=>e(GD,{onClick:()=>i(T),get selected(){return T===u()},children:T})})}})}})]}})};GN(["click"]);const BD=L('<input type="checkbox">'),FD=L('<div class="slider"><div class="toggle"></div></div>'),XD=A("button")`
	background: unset;
	border: unset;
	outline: unset;

	input {
		height: 0;
		width: 0;
		opacity: 0;
		z-index: -2;
	}

	.slider {
		cursor: pointer;
		width: 52px;
		height: 30px;
		border-radius: 34px;
		border-color: #ccc;
		background-color: ${M=>M.theme.colors.bright};
		border: 1px solid ${M=>M.theme.colors.shade};
		display: inline-flex;
		align-items: center;
		padding: 0 4px;
  	transition: .4s;
	}

	.slider .toggle {
		height: 20px;
		width: 20px;
		border-radius: 50%;
		background-color: ${M=>M.theme.colors.bright};
		border: 1px solid ${M=>M.theme.colors.shade};
  	transition: .4s;
	}

	input:checked + .slider {
		background-color: ${M=>M.theme.colors.accent};
  	transition: .4s;
	}

	input:disabled + .slider {
		background-color: ${M=>M.theme.colors.shade};
	}

	input:disabled + .slider .toggle {
		background-color: ${M=>M.theme.colors.shade};
		border: 1px solid ${M=>M.theme.colors.bright};
	}

	input:checked:disabled + .slider .toggle {
		background-color: ${M=>M.theme.colors.bright};
	}

	input:checked + .slider .toggle {
		transform: translateX(22px);
  	transition: .4s;
	}
`,cM=({disabled:M=!1,checked:N=!1})=>{const[D,g]=b(N);return e(XD,{onClick:()=>{M||g(z=>!z)},get children(){return[(()=>{const z=BD.cloneNode(!0);return z.disabled=M,E(()=>z.checked=D()),z})(),FD.cloneNode(!0)]}})},_D=A("div")`
	border: 6px solid #f3f3f3;
  border-radius: 50%;
  border-top: 6px solid ${M=>M.theme.colors[M.type]};
  border-bottom: 6px solid ${M=>M.theme.colors[M.type]};
  border-left: 6px solid ${M=>M.theme.colors[M.type]};
  width: 44px;
  height: 44px;
  -webkit-animation: spin 2s linear infinite;
  animation: spin 2s linear infinite;

@-webkit-keyframes spin {
  0% { -webkit-transform: rotate(0deg); }
  100% { -webkit-transform: rotate(360deg); }
}

@keyframes spin {
  0% { transform: rotate(0deg); }
  100% { transform: rotate(360deg); }
}
`,nM=({type:M="accent"})=>e(_D,{type:M});A("span")`
	display: inline-flex;
	font-size: 14px;
	padding: 8px;
	align-items: center;
	justify-content: space-around;
	min-width: 50px;
	background: ${M=>M.theme.colors[M.type]};
	color: ${M=>M.theme.colors[M.textColor]};
	box-shadow: rgba(99, 99, 99, 0.2) 0px 2px 8px 0px;
	border-radius: 17px;
`;const HD=A("div")`
  margin-left: auto;
  margin-right: auto;
  width: ${M=>M.type==="full"?"100%":"80%"};
  display: ${M=>M.flex?"flex":"block"};
  flex-direction: ${M=>M.flexDirection?M.flexDirection:"row"};
  justify-content: ${M=>M.justifyContent};
  align-items: ${M=>M.alignItems};
	gap: ${M=>M.gap};
	flex-wrap: ${M=>M.flexWrap};
	padding: ${M=>M.padding};
`,o=({type:M,flex:N,flexDirection:D,alignItems:g="stretch",justifyContent:j="flex-start",gap:z="0px",flexWrap:u="no-wrap",padding:t="8px 0",children:i})=>e(HD,{alignItems:g,justifyContent:j,type:M,flex:N,flexDirection:D,gap:z,flexWrap:u,padding:t,children:i});var KD="data:image/svg+xml;base64,";const qD={colors:{accent:"#0880AE",warning:"#F2AC57",success:"#14A38B",error:"#FF7171",primary:"#2C2738",secondary:"#756F86",muted:"#7C9CBF",bright:"#FFFFFF",shade:"#DBE2EA",tint:"#EBF4F8"}},Me=A("div")`
	margin-left: auto;
  margin-right: auto;
  width: 80%;
  height: 50px;
  border-bottom: 1px solid ${qD.colors.shade};
  margin-top: 20px;
`,Ne=M=>M<10?`0${M}`:`${M}`,U=({title:M,rank:N})=>e(Me,{get children(){return e(V,{size:5,weight:"bold",type:"primary",get children(){return[EN(()=>Ne(N)),". ",M]}})}}),rM=A("div")`
	display: flex;
	flex-wrap: wrap;
	gap: 8px;
`,De=()=>e(o,{type:"fluid",flex:!0,gap:"16px",flexWrap:"wrap",flexDirection:"column",justifyContent:"flex-start",get children(){return[e(rM,{get children(){return[e(y,{children:"Accent button"}),e(y,{variant:"ghost",children:"Ghost button"}),e(y,{variant:"bright",children:"Bright button"})]}}),e(rM,{get children(){return[e(y,{disabled:!0,children:"Accent disabled button"}),e(y,{variant:"ghost",disabled:!0,children:"Ghost disabled button"}),e(y,{variant:"bright",disabled:!0,children:"Bright disabled button"})]}}),e(rM,{get children(){return[e(y,{small:!0,children:"Accent small button"}),e(y,{variant:"ghost",small:!0,children:"Ghost small button"}),e(y,{variant:"bright",small:!0,children:"Bright small button"})]}}),e(rM,{get children(){return[e(y,{small:!0,disabled:!0,children:"Accent disabled small button"}),e(y,{variant:"ghost",small:!0,disabled:!0,children:"Ghost disabled small button"}),e(y,{variant:"bright",small:!0,disabled:!0,children:"Bright disabled small button"})]}})]}}),ee={colors:{accent:"#0880AE",warning:"#F2AC57",success:"#14A38B",error:"#FF7171",primary:"#2C2738",secondary:"#756F86",muted:"#7C9CBF",bright:"#FFFFFF",shade:"#DBE2EA",tint:"#EBF4F8"}},ge=()=>e(o,{type:"fluid",flex:!0,gap:"16px",flexWrap:"wrap",get children(){return[e(v,{type:"bright",textColor:"accent",get iconColor(){return ee.colors.accent},children:"A bright alert flash for dark backgrounds, which never lose the contrast."}),e(v,{type:"primary",children:"A dark (primary type) alert flash for bright backgrounds, which never lose the contrast."}),e(v,{type:"success",children:"A success alert flash, which never lose the contrast."}),e(v,{type:"warning",children:"A warning alert flash that never sucks."}),e(v,{type:"error",children:"An error alert flash that nobody loves."}),e(v,{type:"accent",children:"An accent alert flash that looks pretty nice."})]}}),je=()=>e(o,{type:"fluid",flex:!0,gap:"16px",flexWrap:"wrap",get children(){return[e(DN,{title:"Callout Title",text:"Supportive text for the callout goes here like a pro, which informs and helps users decide what they should do next.",get actions(){return[e(y,{small:!0,children:"Action"}),e(y,{variant:"ghost",small:!0,children:" Action"})]}}),e(DN,{text:"Supportive text for the callout.",get actions(){return[e(y,{small:!0,children:"Action"}),e(y,{variant:"ghost",small:!0,children:" Action"})]},small:!0})]}}),ze=()=>e(o,{type:"fluid",flex:!0,gap:"16px",flexWrap:"wrap",get children(){return[e(nM,{type:"accent"}),e(nM,{type:"error"}),e(nM,{type:"warning"}),e(nM,{type:"success"})]}}),te=()=>e(o,{type:"fluid",flex:!0,gap:"16px",flexWrap:"wrap",get children(){return[e(eN,{imageSrc:"https://i.pinimg.com/originals/d4/62/aa/d462aa293e280254708a910f8328eb78.jpg",title:"Card title",get actions(){return[e(y,{variant:"ghost",children:"Action"})]},children:"Supporting description for the card goes here like a breeze."}),e(eN,{title:"Card title",get actions(){return[e(y,{variant:"ghost",children:"Action"})]},children:"Supporting description for the card goes here like a breeze."})]}}),ue=dM("<div></div>"),Te=["primary","accent","error","success","warning","secondary","muted","bright"],ie=[1,2,3,4,5,6],Ie=()=>e(o,{type:"fluid",flex:!0,gap:"16px",flexDirection:"row",flexWrap:"wrap",get children(){return e($,{each:Te,children:M=>(()=>{const N=ue.cloneNode(!0);return n(N,e($,{each:ie,children:D=>e(V,{size:D,type:M,children:`Heading x${D}`})}),null),n(N,e(NM,{type:M,children:"Paragraph x1"}),null),n(N,e(NM,{size:2,type:M,children:"Paragraph x2"}),null),n(N,e(MN,{type:M,children:"Label"}),null),N})()})}}),Ae=()=>e(o,{type:"fluid",flex:!0,flexWrap:"wrap",gap:"8px",get children(){return[e(C,{initials:"RK"}),e(C,{initials:"RK",round:!0}),e(C.Meg,{}),e(C.Meg,{round:!0}),e(C.Mike,{}),e(C.Mike,{round:!0}),e(C.Steven,{}),e(C.Steven,{round:!0}),e(C.Mili,{}),e(C.Mili,{round:!0})]}}),{Cross:ce,More:ne,Plus:re,Minus:ye,Burger:le,Lens:Oe,ChevronLeft:oe,ChevronDown:Le}=Z,xe=()=>e(o,{type:"fluid",flex:!0,gap:"16px",flexDirection:"row",flexWrap:"wrap",get children(){return[e(ce,{}),e(ne,{}),e(re,{}),e(ye,{}),e(le,{}),e(Oe,{}),e(oe,{}),e(Le,{})]}}),se=A("div")`
	background-color: ${M=>M.backgroundColor};
	height: 240px;
	width: 260px;
	border-radius: 40px;
	padding: 40px;
	box-shadow: rgba(99, 99, 99, 0.2) 0px 2px 8px 0px;
`,Ue=A("div")`
	width: 100%;
	height: 70%;
`,Ee=({backgroundColor:M="#2C2738",children:N})=>e(se,{backgroundColor:M,get children(){return[e(Ue,{}),N]}}),ae=[{backgroundColor:"#0880AE",label:"accent",labelType:"bright",headingType:"bright"},{backgroundColor:"#F2AC57",label:"warning",labelType:"bright",headingType:"bright"},{backgroundColor:"#14A38B",label:"success",labelType:"bright",headingType:"bright"},{backgroundColor:"#FF7171",label:"error",labelType:"bright",headingType:"bright"},{backgroundColor:"#2C2738",color:"#ffffff",label:"primary",labelType:"bright",headingType:"bright"},{backgroundColor:"#FFFFFF",label:"bright",labelType:"primary",headingType:"primary"},{backgroundColor:"#DBE2EA",color:"#2C2738",label:"shade",labelType:"primary",headingType:"primary"},{backgroundColor:"#EBF4F8",label:"tint",labelType:"primary",headingType:"primary"}],Ce=()=>e(o,{type:"fluid",flex:!0,flexWrap:"wrap",gap:"8px",justifyContent:"space-evenly",get children(){return e($,{each:ae,children:M=>e(Ee,{get backgroundColor(){return M.backgroundColor},get children(){return[e(MN,{get type(){return M.labelType},get children(){return M.label}}),e(V,{size:4,get type(){return M.headingType},get children(){return M.backgroundColor}})]}})})}}),de=()=>{const[M,N]=b(!1);return e(o,{type:"fluid",flex:!0,gap:"16px",flexWrap:"wrap",get children(){return[e(y,{variant:"ghost",small:!0,onClick:()=>N(!0),children:"Open modal"}),e(YD,{title:"Modal Title",visible:M,onOk:()=>N(!1),onCancel:()=>N(!1),children:"Left aligned contextual description for modal."})]}})},Qe=()=>e(o,{type:"fluid",flex:!0,gap:"16px",flexDirection:"row",flexWrap:"wrap",get children(){return[e(o,{type:"full",flex:!0,gap:"16px",flexDirection:"row",flexWrap:"wrap",get children(){return[e(DM,{}),e(DM,{value:"Value"}),e(DM,{placeholder:"Placeholder"}),e(DM,{value:"Disabled",disabled:!0}),e(DM,{placeholder:"With icon",get icon(){return e(Z.Lens,{})}})]}}),e(o,{type:"full",flex:!0,gap:"16px",flexDirection:"row",flexWrap:"wrap",get children(){return[e(eM,{}),e(eM,{value:"Value"}),e(eM,{placeholder:"Placeholder"}),e(eM,{placeholder:"Disabled",disabled:!0}),e(eM,{placeholder:"Six rows textarea",rows:6})]}}),e(o,{type:"full",flex:!0,gap:"16px",flexDirection:"row",flexWrap:"wrap",get children(){return[e(kM,{defaultValue:6}),e(kM,{defaultValue:1,minValue:-2,maxValue:2}),e(kM,{defaultValue:2,disabled:!0})]}}),e(o,{type:"full",flex:!0,gap:"16px",flexDirection:"row",flexWrap:"wrap",get children(){return[e(cM,{}),e(cM,{checked:!0}),e(cM,{disabled:!0}),e(cM,{checked:!0,disabled:!0})]}}),e(o,{type:"full",flex:!0,gap:"16px",flexDirection:"row",flexWrap:"wrap",get children(){return[e(v,{type:"warning",children:"[Select]: Skeleton of component only! Not fully functional!"}),e(B,{options:["Item 1","Item 2","Item 3"]}),e(B,{options:["Item 1","Item 2","Item 3"],placeholder:"Select placeholder"}),e(B,{options:["Item 1","Item 2","Item 3"],defaultOption:"Item 1"}),e(B,{options:["Item 1","Item 2","Item 3"],disabled:!0}),e(B,{options:["Item 1","Item 2","Item 3"],placeholder:"Select disabled placeholder",disabled:!0}),e(B,{options:["Item 1","Item 2","Item 3"],defaultOption:"Item 1",disabled:!0})]}})]}}),we=()=>e(o,{type:"fluid",flex:!0,gap:"16px",flexWrap:"wrap",get children(){return[e(gM,{type:"accent",percent:20}),e(gM,{type:"error",percent:80}),e(gM,{type:"warning",percent:40}),e(gM,{type:"success",percent:100}),e(gM,{loading:!0})]}}),ke=dM('<img alt="RevkitUI" width="100%">'),Se=dM("<div></div>"),he=()=>(()=>{const M=Se.cloneNode(!0);return M.style.setProperty("height","80%"),n(M,e(o,{type:"full",padding:"0",get children(){const N=ke.cloneNode(!0);return JM(N,"src",KD),N}}),null),n(M,e(U,{title:"Colors",rank:1}),null),n(M,e(Ce,{}),null),n(M,e(U,{title:"Icons",rank:2}),null),n(M,e(xe,{}),null),n(M,e(U,{title:"Form",rank:3}),null),n(M,e(Qe,{}),null),n(M,e(U,{title:"Buttons",rank:5}),null),n(M,e(De,{}),null),n(M,e(U,{title:"Avatars",rank:6}),null),n(M,e(Ae,{}),null),n(M,e(U,{title:"Type Scale",rank:7}),null),n(M,e(Ie,{}),null),n(M,e(U,{title:"Cards",rank:8}),null),n(M,e(te,{}),null),n(M,e(U,{title:"Alerts",rank:9}),null),n(M,e(ge,{}),null),n(M,e(U,{title:"Spinners",rank:10}),null),n(M,e(ze,{}),null),n(M,e(U,{title:"Progress",rank:11}),null),n(M,e(we,{}),null),n(M,e(U,{title:"Callouts",rank:12}),null),n(M,e(je,{}),null),n(M,e(U,{title:"Modals",rank:13}),null),n(M,e(de,{}),null),M})();CN(()=>e($D,{get children(){return e(he,{})}}),document.getElementById("root"))});
